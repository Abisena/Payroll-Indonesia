...
class BPJSSettings(Document):
    ...
    def validate_percentages(self):
        ...
        for rule in percentage_ranges:
            ...
            error_msg = rule.get("error_msg", f"{frappe.unscrub(field)} must be between {min_val} and {max_val}%")
    ...
    def validate_max_salary(self):
        ...
        for rule in salary_thresholds:
            field = rule.get("field")
            min_val = rule.get("min", 0)
            error_msg = rule.get("error_msg", f"{frappe.unscrub(field)} must be greater than {min_val}")
