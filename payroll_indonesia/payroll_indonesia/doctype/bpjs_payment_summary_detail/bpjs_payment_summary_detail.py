    ...
    def _trigger_parent_recalculation(self) -> None:
        ...
        if self.parent and frappe.db.exists("BPJS Payment Summary", self.parent):
            try:
                ...
                logger.error(
                    f"Error triggering recalculation for {self.parent}: {str(e)}"
                )
...
def bulk_sync(slips: List[Union[str, Dict[str, Any], Document]]) -> List[Dict[str, Any]]:
    ...
    if not frappe.db.table_exists("Salary Slip") or not frappe.db.table_exists(
            "Salary Detail"):
    ...
    try:
        ...
        from ..bpjs_payment_summary.bpjs_payment_utils import (
            extract_bpjs_data_from_components
        )
