...
class BPJSPaymentValidator:
    ...
    def _validate_overlap_period(doc: Any) -> None:
        ...
        if existing:
            frappe.throw(
                _("A BPJS Payment Summary already exists for period {0}/{1} in company {2}: {3}")
    ...
    def _validate_component_vs_account(doc: Any) -> None:
        ...
        if missing_accounts:
            frappe.throw(
                _("The following component types do not have corresponding account details: {0}")
        ...
        if abs(component_total - account_total) > 1.0:
            frappe.throw(
                _("Component total ({0}) does not match account detail total ({1}). "
                  "Please reconcile.")
                .format(
                    frappe.utils.fmt_money(component_total),
                    frappe.utils.fmt_money(account_total)
                )
            )
    ...
    def _validate_totals(doc: Any) -> None:
        ...
        if hasattr(doc, "komponen") and doc.komponen:
            ...
            if abs(flt(doc.total) - calculated_total) > 0.1:  # Small tolerance for rounding
                frappe.throw(
                    _("Document total ({0}) does not match calculated total ({1})")
                    .format(
                        frappe.utils.fmt_money(doc.total),
                        frappe.utils.fmt_money(calculated_total)
                    )
                )
    ...
    def _validate_required_accounts(company: str) -> None:
        ...
        if missing_accounts:
            frappe.throw(
                _("Missing required accounts for company {0}: {1}")
                .format(company, ", ".join(missing_accounts))
            )
