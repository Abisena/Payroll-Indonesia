...
from payroll_indonesia.payroll_indonesia.doctype.bpjs_payment_summary.bpjs_payment_utils import (
    safe_decimal, format_reference, get_month_name, calculate_period_dates,
    PayableLine, ExpenseLine, collect_payable_lines, compute_employer_expense,
    get_payment_accounts, debug_log
)
...
class PaymentSummaryService:
    ...
    def __init__(self, doc_or_name: Union[str, Any]):
        ...
        self.company_abbr = frappe.get_cached_value(
    ...
    def create_payment_entry(self) -> Optional[str]:
        ...
        try:
            ...
            if not default_bank:
                frappe.throw(_("Default Bank Account not set for company {0}").format(self.doc.company))
    ...
    def create_employer_journal(self) -> Optional[str]:
        ...
        try:
            ...
            cost_center = frappe.db.get_value(
                "Company", self.doc.company, "cost_center")
    ...
    def _get_bpjs_account_mapping(self) -> Dict[str, Any]:
        ...
        mapping = frappe.get_all(
            ...
            filters={"company": self.doc.company},
            fields=["*"],
            limit=1
        )
    ...
    def _get_account_for_bpjs_type(
        self, bpjs_type: str, mapping: Dict[str, Any]
    ) -> Optional[str]:
        """
        ...
        if bpjs_type in ["JHT", "JP", "JKK", "JKM"]:
            if "bpjs_ketenagakerjaan_payable_account" in mapping and mapping["bpjs_ketenagakerjaan_payable_account"]:
