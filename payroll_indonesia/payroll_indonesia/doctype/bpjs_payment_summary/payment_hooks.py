...
def on_payment_entry_submit(payment_entry: Any, method: Optional[str] = None) -> None:
    ...
    for ref in payment_entry.references:
        if (hasattr(ref, "reference_doctype") and ref.reference_doctype == "BPJS Payment Summary" 
                and hasattr(ref, "reference_name") and ref.reference_name):
    ...
    for summary_name in bpjs_references:
        try:
            ...
            summary_status = frappe.db.get_value(
                "BPJS Payment Summary", summary_name, "docstatus")
