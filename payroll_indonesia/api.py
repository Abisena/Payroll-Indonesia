...
from payroll_indonesia.payroll_indonesia.bpjs_payment_summary.bpjs_payment_services import (
    BPJSPaymentService,
...
def create_payment_summary(
    ...
    try:
        ...
        if year < 2000 or year > 2100:
            return send_response(
                "error",
                error="Year must be valid (between 2000 and 2100)",
                http_status_code=400
            )
        ...
        if isinstance(summary_details, str):
            try:
                ...
                return send_response(
                    "error",
                    error="Invalid JSON format for summary_details",
                    http_status_code=400
                )
...
def get_payment_status(name: str) -> Dict[str, Any]:
    ...
    try:
        if not name:
            return send_response(
                "error",
                error="BPJS Payment Summary ID is required",
                http_status_code=400
            )
...
def get_employee_bpjs_data(
    ...
    try:
        ...
        if not employee and not company:
            return send_response(
                "error",
                error="Either employee or company is required",
                http_status_code=400
            )
