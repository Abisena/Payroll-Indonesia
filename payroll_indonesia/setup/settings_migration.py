...
def migrate_ptkp(settings: "frappe.Document", config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        for status_code, amount in ptkp_data.items():
            settings.append("ptkp_table", {"status_code": status_code, "amount": flt(amount)})
            count += 1
...
def migrate_tax_brackets(settings: "frappe.Document", config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if not hasattr(settings, "tax_brackets_table"):
            debug_log(
                "tax_brackets_table child table not found in Payroll Indonesia Settings", "warning"
            )
            return False
...
def migrate_ter_rates(settings: "frappe.Document", config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        has_json_backup = (
            hasattr(settings, "ter_rate_ter_a_json")
            or hasattr(settings, "ter_rate_ter_b_json")
            or hasattr(settings, "ter_rate_ter_c_json")
        )
    ...
    try:
        ...
        if validation_limits:
            ...
            for field_name, default_value in limit_fields:
                if hasattr(settings, field_name):
                    setattr(
                        settings, field_name, flt(validation_limits.get(field_name, default_value))
                    )

            # Set calculation precision and rounding method
            if hasattr(settings, "bpjs_calculation_precision"):
                settings.bpjs_calculation_precision = cint(
                    validation_limits.get("calculation_precision", 2)
                )
                ...
                if config_key in struktur_gaji:
                    if setting_field == "working_days_default":
                        setattr(
                            settings,
                            setting_field,
                            cint(struktur_gaji.get(config_key, default_value)),
                        )
                    else:
                        setattr(
                            settings,
                            setting_field,
                            flt(struktur_gaji.get(config_key, default_value)),
                        )
            ...
            for setting_field, config_key, default_value, convert_func in default_mappings:
                if hasattr(settings, setting_field) and config_key in defaults:
                    setattr(
                        settings,
                        setting_field,
                        convert_func(defaults.get(config_key, default_value)),
                    )
...
def migrate_ptkp_ter_mapping(settings: "frappe.Document", config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if not hasattr(settings, "ptkp_ter_mapping_table"):
            debug_log(
                "ptkp_ter_mapping_table child table not found in Payroll Indonesia Settings",
                "warning",
            )
            return False
        ...
        for ptkp_code, ter_category in mapping_data.items():
            settings.append(
                "ptkp_ter_mapping_table", {"ptkp_code": ptkp_code, "ter_category": ter_category}
            )
            count += 1
    ...
    try:
        ...
        for setting_field, config_key, default_value, convert_func in field_mappings:
            if hasattr(settings, setting_field) and config_key in tax_config:
                setattr(
                    settings, setting_field, convert_func(tax_config.get(config_key, default_value))
                )
            ...
            for field_name, default_value in limit_mappings:
                if hasattr(settings, field_name):
                    if field_name == "tax_rounding_precision":
                        setattr(
                            settings, field_name, cint(tax_limits.get(field_name, default_value))
                        )
                    else:
                        setattr(
                            settings, field_name, flt(tax_limits.get(field_name, default_value))
                        )
...
def migrate_employee_types(settings: "frappe.Document", config: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if not hasattr(settings, "tipe_karyawan_table"):
            debug_log(
                "tipe_karyawan_table child table not found in Payroll Indonesia Settings", "warning"
            )
            return False
