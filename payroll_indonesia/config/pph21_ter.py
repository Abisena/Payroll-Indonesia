...
def calculate_pph21_TER(employee_doc, salary_slip):
    ...
    # 3) Pengurang netto dan biaya jabatan (hanya dari komponen yang ada)
    pengurang_netto = sum_pengurang_netto(salary_slip)
    biaya_jabatan = get_biaya_jabatan_from_component(salary_slip) or 0.0

    # 4) PTKP bulanan
    try:
        ptkp_monthly = get_ptkp_amount(employee_doc) / 12
    except ValidationError as e:
        frappe.logger().warning(str(e))
        ptkp_monthly = 0.0

    # 5) Taxable income dan tarif TER
    taxable_income = bruto - pengurang_netto - biaya_jabatan - ptkp_monthly
